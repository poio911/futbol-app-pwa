...
const TestApp = {
    ...
    async loadInitialData() {
        ...
        if (this.currentGroup) {
            ...
            const activeGroupElement = document.getElementById('active-group-name');
            if (currentGroupElement) {
                currentGroupElement.textContent = this.currentGroup.name || 'Group';
            }
            if (activeGroupElement) {
                activeGroupElement.textContent = this.currentGroup.name || 'None';
            }
    ...
    // Display players - SIMPLIFIED SYSTEM
    displayPlayers() {
        console.log('🎨 TestApp.displayPlayers called with', this.players.length, 'players');
        
        // SINGLE SYSTEM: Use Enhanced Players View only
        ...
        // Fallback if Enhanced View not available
        console.log('📝 Using basic fallback rendering');
        this.displayPlayersBasic();
    },

    // Basic fallback rendering - SIMPLIFIED
    displayPlayersBasic() {
    ...
    calculateOVR() {
        ...
        // Use Storage's unified OVR calculation
        const attributes = { pac, sho, pas, dri, def, phy };
        const ovr = Storage && Storage.calculateUnifiedOVR 
            ? Storage.calculateUnifiedOVR(attributes, position)
            : Math.round((pac + sho + pas + dri + def + phy) / 6); // Fallback
    ...
    initMatchScreen() {
        try {
            ...
            const savedMatch = localStorage.getItem('activeManualMatch');
            if (savedMatch) {
                try {
                    this.currentMatch = JSON.parse(savedMatch);
                    console.log('✅ Restored active match from localStorage:', this.currentMatch.id);
                    
                    // Restore the match display if in manual matches view
                    if (document.querySelector('.partidos-manuales-content')?.style.display !== 'none') {
                        this.displayUnifiedTeams(this.currentMatch);
                        document.getElementById('match-actions-generated').style.display = 'block';
                        console.log('✅ Active match display restored');
                    }
                } catch (error) {
                    console.error('Error restoring active match:', error);
        ...
        modal.innerHTML = `
            ...
            ">
                ...
                <div style="margin-bottom: 20px; text-align: center;">
                    ...
                    ">0 jugadores seleccionados (mínimo 6)</div>
                ...
                ">
                    ${this.players.map(player => `
                        ...
                        onmouseover="if(!this.classList.contains('selected')) { this.style.borderColor='rgba(0, 255, 157, 0.4)'; this.style.background='rgba(0, 255, 157, 0.05)'; this.style.transform='translateY(-2px)'; }"
                        onmouseout="if(!this.classList.contains('selected')) { this.style.borderColor='rgba(0, 255, 157, 0.2)'; this.style.background='rgba(40, 40, 40, 0.6)'; this.style.transform='translateY(0)'; }">
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <div style="
                                    width: 50px;
                                    height: 50px;
                                    border-radius: 50%;
                                    background: ${player.photo ? `url(${player.photo})` : 'linear-gradient(135deg, var(--primary), var(--secondary))'};
                                    ...
                                    justify-content: center;
                                    font-weight: bold;
                                    color: ${player.photo ? 'transparent' : '#000'};
                                    font-size: 20px;
    ...
    togglePlayerSelection(card, playerId) {
        ...
        if (this.selectedPlayers.has(playerId)) {
            ...
            card.style.background = 'rgba(40, 40, 40, 0.6)';
            card.style.boxShadow = '0 2px 8px rgba(0, 0, 0, 0.2)';
            card.style.transform = 'translateY(0)';
            indicator.style.background = 'transparent';
            indicator.style.borderColor = 'rgba(0, 255, 157, 0.3)';
            ...
            card.style.background = 'rgba(0, 255, 157, 0.15)';
            card.style.boxShadow = '0 4px 16px rgba(0, 255, 157, 0.3)';
        ...
        countElement.textContent = `${count} jugadores seleccionados${count < 6 ? ' (mínimo 6)' : count > 20 ? ' (máximo 20)' : ''}`;
        ...
        if (count >= 6 && count <= 20) {
            ...
            countElement.style.textShadow = '0 0 10px rgba(0, 255, 157, 0.5)';
            ...
            countElement.style.color = count < 6 ? '#ff4444' : '#ffaa00';
            countElement.style.textShadow = count < 6 ? '0 0 10px rgba(255, 68, 68, 0.5)' : '0 0 10px rgba(255, 170, 0, 0.5)';
    ...
    async generateTeamsWithPlayers(playersToUse) {
        ...
        try {
            ...
            // Save active match to localStorage for persistence
            localStorage.setItem('activeManualMatch', JSON.stringify(this.currentMatch));
            console.log('✅ Active match saved to localStorage for persistence across reloads');
            
            // Display teams using unified system
            this.displayUnifiedTeams(this.currentMatch);
            
            // Show match actions with buttons
            const matchActionsDiv = document.getElementById('match-actions-generated');
            if (matchActionsDiv) {
                matchActionsDiv.style.display = 'block';
                matchActionsDiv.innerHTML = `
                    <div style="
                        padding: 20px;
                    ...
                    ">
                        ...
                        <div style="
                            ...
                            flex-wrap: wrap;
                        ">
                            <button onclick="TestApp.showTeamsModal()" class="btn-unified btn-secondary" style="
                                ...
                                transition: transform 0.3s;
                            ">
                                👁️ Ver Equipos Detallados
                            </button>
                            <button onclick="TestApp.saveMatch()" class="btn-unified btn-primary" style="
                            ...
                            <button onclick="TestApp.regenerateTeams()" class="btn-unified" style="
    ...
    async saveMatch() {
        ...
        try {
            ...
            if (db) {
                ...
                // Crear notificaciones usando el sistema unificado
                if (window.notificationsSystem) {
                    ...
                    window.notificationsSystem.showToast(
                        `Partido "${matchData.name}" creado exitosamente 🏆`,
                        'success'
                    );
                }
            } else {
            ...
            if (window.notificationsSystem) {
                window.notificationsSystem.showModal({
                    ...
                    message: '¡Partido guardado exitosamente! Revisa el historial abajo.',
                    type: 'success'
                });
            } else {
            ...
            if (window.notificationsSystem) {
                window.notificationsSystem.showModal({
                    title: '❌ Error',
                    message: `Error guardando partido: ${error.message}`,
                    type: 'error'
                });
            } else {
    ...
    clearMatchDisplay() {
        ...
        // Clear current match and localStorage
    ...
    async applyPlayerImprovements() {
        ...
        for (const playerId in this.currentEvaluation.playerPerformances) {
            ...
            if (totalImprovement > 0) {
                ...
                // USAR SISTEMA UNIFICADO con trazabilidad completa
                console.log('🔍 STORAGE DEBUG:', {
                    storageExists: !!Storage,
                    updatePlayerUnifiedExists: !!(Storage && Storage.updatePlayerUnified),
                    updatePlayerExists: !!(Storage && Storage.updatePlayer),
                    storageMethods: Storage ? Object.keys(Storage).filter(key => key.includes('update')) : []
                });
                
                if (Storage && Storage.updatePlayerUnified) {
                    // Preparar updates para sistema unificado (campos directos)
                    ...
                    // Contexto para trazabilidad completa
                    const evaluationContext = {
                        matchId: this.getCurrentMatchId(),
                        evaluatorId: this.getCurrentEvaluatorId(),
                        evaluationType: this.evaluationSystem,
                        evaluationData: {
                    ...
                    console.log('🎯 Using unified system for player update:', {
                        player: player.name,
                        updates: unifiedUpdates,
                        context: evaluationContext
                    });
                    
                    await Storage.updatePlayerUnified(player.id, unifiedUpdates, evaluationContext);
                    this.log(`✅ Player updated via UNIFIED system: ${player.name}`, 'success');
                    
                } else if (Storage && Storage.updatePlayer) {
                    // FALLBACK a sistema legacy si unified no está disponible
                    console.log('⚠️ Falling back to legacy updatePlayer for:', player.name);
                    await Storage.updatePlayer(player);
                    this.log(`⚠️ Player updated via legacy system: ${player.name}`, 'warning');
                } else {
                    this.log(`❌ No storage system available for: ${player.name}`, 'error');
                }
    ...
    // Helper functions for unified system
    getCurrentMatchId() {
        if (this.currentEvaluation && this.currentEvaluation.match) {
            return this.currentEvaluation.match.id || 'unknown-match';
        }
        return 'no-match-context';
    },
    
    getCurrentEvaluatorId() {
        // Try to get current user ID from different sources
        if (AuthSystem && AuthSystem.currentUser && AuthSystem.currentUser.uid) {
            return AuthSystem.currentUser.uid;
        }
        if (Storage && Storage.currentPersonId) {
            return Storage.currentPersonId;
        }
    ...
    calculatePositionBasedOVR(attributes, position) {
        ...
        return Math.min(99, Math.max(1, Math.round((pac + sho + pas + dri + def + phy) / 6)));
    },
    ...
    // Show teams modal with detailed view
    showTeamsModal() {
        if (!this.currentMatch) {
            alert('❌ No hay equipos generados para mostrar');
            return;
        }
        
        // Use the unified modal system if available
        if (window.UnifiedTeamsModal && window.UnifiedTeamsModal.show) {
            window.UnifiedTeamsModal.show(this.currentMatch);
        } else {
            // Fallback to basic display
            alert('Vista detallada de equipos:\n\n' + 
                  `${this.currentMatch.teamA.name}: ${this.currentMatch.teamA.ovr} OVR\n` +
                  `${this.currentMatch.teamB.name}: ${this.currentMatch.teamB.ovr} OVR\n\n` +
                  `Diferencia: ${this.currentMatch.difference} puntos`);
        }
    },
    ...
    async saveMatch() {
        ...
        try {
            ...
            // First check if the match exists in Firebase
            if (typeof db !== 'undefined' && db) {
                ...
                if (doc.exists) {
                    ...
                    console.log('✅ Match exists in Firebase, updating:', this.currentMatch.id);
                    await docRef.update({
                        status: 'active',
                        updatedAt: new Date().toISOString()
                    });
                    ...
                    console.log('📝 Match not in Firebase, creating:', this.currentMatch.id);
                    await docRef.set(matchData);
                }
                console.log('✅ Match saved successfully:', this.currentMatch.id);
                
                // Remove from localStorage since it's finished
                localStorage.removeItem('activeManualMatch');
                console.log('✅ Active match removed from localStorage after finishing');
                
                // Hide teams display after updating
                const teamsContainer = document.getElementById('teams-display');
                if (teamsContainer) {
                    teamsContainer.innerHTML = '';
