...
const AuthSystem = {
    ...
    PREVENT_AUTO_LOGIN: true, // Prevent automatic login on page load
    ...
    async init() {
        ...
        // Check if user explicitly wants to stay logged out
        const stayLoggedOut = localStorage.getItem('auth_stay_logged_out') === 'true';
        if (stayLoggedOut) {
            console.log('🚫 User chose to stay logged out');
            this.showAuthScreen();
            return;
        }
        
        // Only check for persisted user if not preventing auto-login
        if (!this.PREVENT_AUTO_LOGIN) {
            ...
            console.log('🔒 Auto-login disabled, showing auth screen');
            this.showAuthScreen();
        ...
        try {
            ...
            authInstance.onAuthStateChanged(async (user) => {
                try {
                    ...
                    if (error.message?.includes('400') || (error.code && error.code.includes('auth/'))) {
    ...
    async register() {
        ...
        try {
            ...
            if (useDirectRegistration) {
                ...
                localStorage.removeItem('auth_stay_logged_out');
    ...
    async createUserProfile(firebaseUser, additionalData = {}) {
        ...
        try {
            ...
            if (window.notificationsSystem) {
                ...
                // Crear notificación para cada usuario usando el sistema unificado
                if (window.notificationsSystem) {
    ...
    handleUserSignedOut() {
        ...
        // Notify HeaderManager about logout
        if (window.headerManager) {
            window.headerManager.updateUser(null);
            window.notifyUserChange(null);
        }
    },
    ...
    // Logout
    async logout(stayLoggedOut = true) {
        console.log('🚪 Starting logout process...');
        
        try {
            // Close session using SessionManager
            if (window.SessionManager) {
                await window.SessionManager.closeSession();
            }
            
            // Force close all sessions for this user across devices
            if (this.currentUser && window.SessionManager) {
                await window.SessionManager.forceCloseAllUserSessions(this.currentUser.uid);
            }
            
            // Clear ALL session data from both storages
            localStorage.setItem('auth_logged_out', 'true');
            if (stayLoggedOut) {
                localStorage.setItem('auth_stay_logged_out', 'true');
            }
            localStorage.removeItem('auth_current_session');
            sessionStorage.removeItem('auth_current_session');
            ...
            try {
                ...
                console.log('Firebase signOut failed:', firebaseError.message);
            }
    ...
    async checkPersistedUser() {
        ...
        // First check if user explicitly logged out and wants to stay logged out
        const stayLoggedOut = localStorage.getItem('auth_stay_logged_out');
        if (stayLoggedOut === 'true') {
            console.log('🚫 User explicitly wants to stay logged out');
            return false;
        }
        
        // Check SessionManager for valid session
        if (window.SessionManager) {
            const session = window.SessionManager.getCurrentSession();
            if (session && session.user) {
                console.log('✅ Valid session found via SessionManager');
                this.currentUser = session.user;
                this.isAuthenticated = true;
                this.hideAuthScreen();
                this.initializeSystemsWithUser(session.user);
                return true;
            }
        }
    ...
    async initializeSystemsWithUser(userData) {
        ...
        // Disparar evento de login exitoso para el header
        const loginEvent = new CustomEvent('userLoggedIn', { 
            detail: { user: userData }
        });
        document.dispatchEvent(loginEvent);
        console.log('📡 Evento userLoggedIn disparado para header');
        ...
        // Update HeaderManager with the new user
        if (window.headerManager) {
            console.log('🎨 Updating HeaderManager with user:', userData.displayName);
            window.headerManager.updateUser(userData);
            window.notifyUserChange(userData);
        }
        
        // Update CleanHeader with the new user
        if (window.cleanHeader) {
            console.log('✨ Updating CleanHeader with user:', userData.displayName);
            window.cleanHeader.updateUser(userData);
        }
        
        // Reinitialize Header & Footer with the new user (legacy)
        if (window.headerFooter) {
            console.log('🎨 Reinitializing legacy header with user:', userData.displayName);
            window.headerFooter.currentUser = userData;
            await window.headerFooter.updateUserData();
        }
        
        // Notifications System se auto-inicializa, solo actualizamos el usuario
        if (window.notificationsSystem) {
            console.log('🔔 Updating notifications user:', userData.displayName);
