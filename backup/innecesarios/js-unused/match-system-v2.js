    ...
    generateBalancedTeams(players, format = '5v5') {
        ...
        if (window.TeamGeneratorAdvanced) {
            try {
                ...
                const result = window.TeamGeneratorAdvanced.generateBalancedTeams(players, format);
                
                // Adaptar al formato esperado por el sistema actual
                return {
                    teamA: {
                        name: result.teamA.name || 'Equipo A',
                        players: result.teamA.players,
                        ovr: result.teamA.ovr
                    },
                    teamB: {
                        name: result.teamB.name || 'Equipo B',
                        players: result.teamB.players,
                        ovr: result.teamB.ovr
                    },
